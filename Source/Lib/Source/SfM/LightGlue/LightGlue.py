...
def ApplyCachedRotaryEmb(freqs: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
...
class LearnableFourierPositionalEncoding(nn.Module):
    def __init__(self, M: int, dim: int, F_dim: int = None, gamma: float = 1.0, device : Optional[torch.device] = None) -> None:
        ...
        self.gamma = gamma
        self.Wr = nn.Linear(M, F_dim // 2, bias = False, device = device)
        if device != "meta":
...
class SelfBlock(nn.Module):
    def __init__(
        ...
        self.Wqkv = nn.Linear(embed_dim, 3 * embed_dim, bias = bias, device = device)
        self.inner_attn = Attention(flash)
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias = bias, device = device)
        self.ffn = nn.Sequential(
            nn.Linear(2 * embed_dim, 2 * embed_dim, device = device),
            nn.LayerNorm(2 * embed_dim, elementwise_affine = True, device = device),
            nn.GELU(),
            nn.Linear(2 * embed_dim, embed_dim, device = device),
        )
    ...
    def forward(
        ...
        q = ApplyCachedRotaryEmb(encoding, q)
        k = ApplyCachedRotaryEmb(encoding, k)
...
class CrossBlock(nn.Module):
    def __init__(
        ...
        self.to_qk = nn.Linear(embed_dim, inner_dim, bias = bias, device = device)
        self.to_v = nn.Linear(embed_dim, inner_dim, bias = bias, device = device)
        self.to_out = nn.Linear(inner_dim, embed_dim, bias = bias, device = device)
        self.ffn = nn.Sequential(
            nn.Linear(2 * embed_dim, 2 * embed_dim, device = device),
            nn.LayerNorm(2 * embed_dim, elementwise_affine = True, device = device),
            nn.GELU(),
...
class MatchAssignment(nn.Module):
    ...
    def forward(self, desc0: torch.Tensor, desc1: torch.Tensor):
        ...
        scores = SigmoidLogDoubleSoftmax(sim, z0, z1)
        return scores, sim
...
class LightGlue(nn.Module):
    ...
    def __init__(self, features = "superpoint", device : Optional[torch.device] = None, **conf) -> None:
        ...
        if conf.input_dim != conf.descriptor_dim:
            self.input_proj = nn.Linear(conf.input_dim, conf.descriptor_dim, bias=True, device = device)
        else:
            self.input_proj = nn.Identity()

        head_dim = conf.descriptor_dim // conf.num_heads
        self.posenc = LearnableFourierPositionalEncoding(
            2 + 2 * self.conf.add_scale_ori, head_dim, head_dim, device = device
        )

        h, n, d = conf.num_heads, conf.n_layers, conf.descriptor_dim

        self.transformers = nn.ModuleList(
            [TransformerLayer(d, h, conf.flash, device = device) for _ in range(n)]
        )

        self.log_assignment = nn.ModuleList([MatchAssignment(d, device = device) for _ in range(n)])
        self.token_confidence = nn.ModuleList(
            [TokenConfidence(d, device = device) for _ in range(n - 1)]
        )
        self.register_buffer(
            "confidence_thresholds",
            torch.Tensor(
                [self.ConfidenceThreshold(i) for i in range(self.conf.n_layers)]
            ),
        )
    ...
    def FromPretrained(cls, path: Union[str, Path], features = "superpoint", **conf) -> "LightGlue":
        ...
        for i in range(lightglue.conf.n_layers):
            pattern = f"self_attn.{i}", f"transformers.{i}.self_attn"
            state_dict = {k.replace(*pattern): v for k, v in state_dict.items()}
            pattern = f"cross_attn.{i}", f"transformers.{i}.cross_attn"
            state_dict = {k.replace(*pattern): v for k, v in state_dict.items()}
        lightglue.load_state_dict(state_dict, strict = False)

        # Refill the buffer to cancel out the skip_init
        lightglue.confidence_thresholds = torch.Tensor(
            [lightglue.ConfidenceThreshold(i) for i in range(lightglue.conf.n_layers)]
        )
    ...
    def compile(
        ...
        for i in range(self.conf.n_layers):
            self.transformers[i].MaskedForward = torch.compile(
                self.transformers[i].MaskedForward, mode = mode, fullgraph = True
            )
    ...
    def _forward(self, data: dict) -> dict:
        ...
        kpts0 = NormalizeKeypoints(kpts0, size0).clone()
        kpts1 = NormalizeKeypoints(kpts1, size1).clone()
        ...
        if torch.is_autocast_enabled():
            desc0 = desc0.to(torch.float16)
            desc1 = desc1.to(torch.float16)
        ...
        if do_compile:
            ...
            desc0, mask0 = PadToLength(desc0, kn)
            desc1, mask1 = PadToLength(desc1, kn)
            kpts0, _ = PadToLength(kpts0, kn)
            kpts1, _ = PadToLength(kpts1, kn)
        ...
        pruning_th = self.PruningMinKpts(device)
        if do_point_pruning:
        ...
        for i in range(self.conf.n_layers):
            ...
            if do_early_stop:
                ...
                if self.CheckIfStop(token0[..., :m], token1[..., :n], i, m + n):
                    break
            if do_point_pruning and desc0.shape[-2] > pruning_th:
                scores0 = self.log_assignment[i].GetMatchability(desc0)
                prunemask0 = self.GetPruningMask(token0, scores0, i)
            ...
            if do_point_pruning and desc1.shape[-2] > pruning_th:
                scores1 = self.log_assignment[i].GetMatchability(desc1)
                prunemask1 = self.GetPruningMask(token1, scores1, i)
        ...
        m0, m1, mscores0, mscores1 = FilterMatches(scores, self.conf.filter_threshold)
        matches, mscores = [], []
        for k in range(b):
            valid = m0[k] > -1
        ...
        if do_point_pruning:
            ...
            new_mscores0 = torch.zeros((b, m), device = mscores0.device)
            new_mscores1 = torch.zeros((b, n), device = mscores1.device)
            new_mscores0[:, ind0] = mscores0
            new_mscores1[:, ind1] = mscores1
            m0, m1, mscores0, mscores1 = m0_, m1_, new_mscores0, new_mscores1
        else:
            prune0 = torch.ones_like(mscores0) * self.conf.n_layers
            prune1 = torch.ones_like(mscores1) * self.conf.n_layers
    ...
    def ConfidenceThreshold(self, layer_index: int) -> float:
