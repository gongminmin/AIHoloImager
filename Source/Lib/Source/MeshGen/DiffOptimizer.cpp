...
namespace AIHoloImager
{
    ...
    #pragma omp parallel
            ...
            auto args = python_system_.MakeTuple(14);
            {
                python_system_.SetTupleItem(*args, 0,
                    python_system_.MakeObject(
                        std::span(reinterpret_cast<std::byte*>(positions.data()), positions.size() * sizeof(glm::vec3))));
                python_system_.SetTupleItem(*args, 1,
                    python_system_.MakeObject(std::span(reinterpret_cast<std::byte*>(colors.data()), colors.size() * sizeof(glm::vec3))));
                ...
                python_system_.SetTupleItem(*args, 3,
                    python_system_.MakeObject(std::span(reinterpret_cast<std::byte*>(indices.data()), indices.size() * sizeof(uint32_t))));
                ...
                {
                    auto img_tuple = python_system_.MakeTuple(5);

                    const auto& delighted_image = sfm_input.views[i].delighted_image;
                    auto image = python_system_.MakeObject(
                        std::span<const std::byte>(reinterpret_cast<const std::byte*>(delighted_image.Data()), delighted_image.DataSize()));
                ...
                python_system_.SetTupleItem(*args, 6, python_system_.MakeObject(sfm_input.views[0].image_mask.Width()));
                python_system_.SetTupleItem(*args, 7, python_system_.MakeObject(sfm_input.views[0].image_mask.Height()));
                ...
                python_system_.SetTupleItem(*args, 8,
                    python_system_.MakeObject(
                        std::span(reinterpret_cast<std::byte*>(view_proj_mtxs.data()), view_proj_mtxs.size() * sizeof(glm::mat4x4))));
                python_system_.SetTupleItem(*args, 9,
                    python_system_.MakeObject(
                        std::span(reinterpret_cast<std::byte*>(transform_offsets.data()), transform_offsets.size() * sizeof(glm::ivec2))));

                python_system_.SetTupleItem(*args, 10, python_system_.MakeObject(num_images));

                python_system_.SetTupleItem(
                    *args, 11, python_system_.MakeObject(std::span(reinterpret_cast<std::byte*>(&scale), sizeof(scale))));
                python_system_.SetTupleItem(
                    *args, 12, python_system_.MakeObject(std::span(reinterpret_cast<std::byte*>(&rotation), sizeof(rotation))));
                python_system_.SetTupleItem(
                    *args, 13, python_system_.MakeObject(std::span(reinterpret_cast<std::byte*>(&translation), sizeof(translation))));
            }
