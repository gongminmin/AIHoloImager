...
namespace AIHoloImager
{
    ...
    public:
        explicit Impl(AIHoloImagerInternal& aihi) : aihi_(aihi), gpu_system_(aihi.GpuSystemInstance())
        {
            ...
            }
            {
                ...
                const GpuVertexLayout vertex_layout(gpu_system_,
                    ...
                    // Although the shader only uses position, the VB contains full data. Specify the stride here.
            ...
            uint32_t texture_size)
        {
            ...
            this->FlattenMesh(cmd_list, mesh_vb, mesh_ib, model_mtx, texture_size, flatten_pos_tex, flatten_normal_tex);
    ...
    private:
            ...
            uint32_t texture_size, GpuTexture2D& flatten_pos_tex, GpuTexture2D& flatten_normal_tex)
        {
            ...
            flatten_pos_tex = GpuTexture2D(gpu_system_, texture_size, texture_size, 1, PositionFmt,
                GpuResourceFlag::ShaderResource | GpuResourceFlag::RenderTarget, "TextureReconstruction.flatten_pos_tex");
            flatten_normal_tex = GpuTexture2D(gpu_system_, texture_size, texture_size, 1, NormalFmt,
                GpuResourceFlag::ShaderResource | GpuResourceFlag::RenderTarget, "TextureReconstruction.flatten_normal_tex");

            GpuRenderTargetView pos_rtv(gpu_system_, flatten_pos_tex);
            GpuRenderTargetView normal_rtv(gpu_system_, flatten_normal_tex);

            GpuConstantBufferOfType<FlattenConstantBuffer> flatten_cb(gpu_system_, "flatten_cb");
            flatten_cb->model_mtx = glm::transpose(model_mtx);
            flatten_cb->model_it_mtx = glm::inverse(model_mtx);
            flatten_cb.UploadStaging();
            const GpuConstantBufferView flatten_cbv(gpu_system_, flatten_cb);

            const float clear_clr[] = {0, 0, 0, 0};
            cmd_list.Clear(pos_rtv, clear_clr);
            cmd_list.Clear(normal_rtv, clear_clr);

            const GpuCommandList::VertexBufferBinding vb_bindings[] = {{&mesh_vb, 0}};
            ...
            std::span<const AIHoloImagerInternal::ProjectionDesc> projections, uint32_t texture_size)
        {
            ...
            GpuTexture2D accum_color_tex(gpu_system_, texture_size, texture_size, 1, GpuFormat::RGBA16_Float,
            ...
            for (size_t i = 0; i < projections.size(); ++i)
            {
                ...
                GpuTexture2D shadow_map_tex(gpu_system_, projection.full_width, projection.full_height, 1, GpuFormat::D32_Float,
                    GpuResourceFlag::ShaderResource | GpuResourceFlag::DepthStencil, "TextureReconstruction.shadow_map_tex");
            ...
            const glm::mat4x4& mvp_mtx, const AIHoloImagerInternal::ProjectionDesc& projection, GpuDepthStencilView& shadow_map_dsv)
        {
            ...
            const GpuConstantBufferView gen_shadow_map_cbv(gpu_system_, gen_shadow_map_cb);
        ...
        GpuTexture2D ResolveTexture(GpuCommandList& cmd_list, uint32_t texture_size, const GpuTexture2D& accum_color_tex)
        {
            ...
            GpuConstantBufferOfType<ResolveTextureConstantBuffer> resolve_texture_cb(gpu_system_, "resolve_texture_cb");
            resolve_texture_cb->texture_size = texture_size;
            resolve_texture_cb.UploadStaging();
            const GpuConstantBufferView resolve_texture_cbv(gpu_system_, resolve_texture_cb);

            std::tuple<std::string_view, const GpuConstantBufferView*> cbvs[] = {
                {"param_cb", &resolve_texture_cbv},
            };
            std::tuple<std::string_view, const GpuShaderResourceView*> srvs[] = {
