...
namespace AIHoloImager
{
    ...
    public:
        explicit Impl(const std::filesystem::path& tmp_dir)
            : exe_dir_(ExeDir()), tmp_dir_(tmp_dir), python_system_(exe_dir_), sfm_(exe_dir_, gpu_system_, python_system_),
              mesh_recon_(exe_dir_), mv_renderer_(gpu_system_, python_system_, 320, 320), mesh_gen_(exe_dir_, gpu_system_, python_system_)
        {
        }
        ...
        Mesh Generate(const std::filesystem::path& input_path)
        {
            ...
            return mesh_gen_.Generate(mv_renderer_result.multi_view_images, 2048, mesh_recon_result, tmp_dir_);
        }
