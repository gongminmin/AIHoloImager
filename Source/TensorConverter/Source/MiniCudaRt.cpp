...
namespace AIHoloImager
{
    ...
    MiniCudaRt::Error_t MiniCudaRt::GetDeviceProperties(DeviceProp* prop, int32_t device) const noexcept
    {
        assert(*this && cuda_get_device_properties_);
        return cuda_get_device_properties_(prop, device);
    }

    MiniCudaRt::ChannelFormatDesc MiniCudaRt::CreateChannelDesc(
        int32_t x, int32_t y, int32_t z, int32_t w, ChannelFormatKind fmt) const noexcept
    {
        assert(*this && cuda_create_channel_desc_);
        return cuda_create_channel_desc_(x, y, z, w, fmt);
    }

    MiniCudaRt::Error_t MiniCudaRt::ImportExternalMemory(
        ExternalMemory_t* ext_mem_out, const ExternalMemoryHandleDesc* mem_handle_desc) const noexcept
    {
        assert(*this && cuda_import_external_memory_);
        return cuda_import_external_memory_(ext_mem_out, mem_handle_desc);
    }

    MiniCudaRt::Error_t MiniCudaRt::DestroyExternalMemory(ExternalMemory_t ext_mem) const noexcept
    {
    ...
    MiniCudaRt::Error_t MiniCudaRt::DestroyExternalSemaphore(ExternalSemaphore_t ext_sem) const noexcept
    {
        assert(*this && cuda_destroy_external_semaphore_);
        return cuda_destroy_external_semaphore_(ext_sem);
    }

    MiniCudaRt::Error_t MiniCudaRt::SignalExternalSemaphoresAsync(const ExternalSemaphore_t* ext_sem_array,
        const ExternalSemaphoreSignalParams* params_array, uint32_t num_ext_sems, Stream_t stream) const noexcept
    {
    ...
    MiniCudaRt::Error_t MiniCudaRt::WaitExternalSemaphoresAsync(const ExternalSemaphore_t* ext_sem_array,
        const ExternalSemaphoreWaitParams* params_array, uint32_t num_ext_sems, Stream_t stream) const noexcept
    {
    ...
    MiniCudaRt::Error_t MiniCudaRt::StreamCreate(Stream_t* stream_out) const noexcept
    {
        assert(*this && cuda_stream_create_);
        return cuda_stream_create_(stream_out);
    }

    MiniCudaRt::Error_t MiniCudaRt::StreamDestroy(Stream_t stream) const noexcept
    {
        assert(*this && cuda_stream_destroy_);
        return cuda_stream_destroy_(stream);
    }

    MiniCudaRt::Error_t MiniCudaRt::MemcpyAsync(void* dst, const void* src, size_t count, MemcpyKind kind, Stream_t stream) const noexcept
    {
        assert(*this && cuda_memcpy_async_);
        return cuda_memcpy_async_(dst, src, count, kind, stream);
    }

    MiniCudaRt::Error_t MiniCudaRt::Memcpy3DAsync(const Memcpy3DParams* p, Stream_t stream) const noexcept
    {
        assert(*this && cuda_memcpy_3d_async_);
        return cuda_memcpy_3d_async_(p, stream);
    }

    MiniCudaRt::Error_t MiniCudaRt::GetMipmappedArrayLevel(
        Array_t* level_array, MipmappedArray_const_t mipmapped_array, uint32_t level) const noexcept
    {
        assert(*this && cuda_get_mipmapped_array_level_);
        return cuda_get_mipmapped_array_level_(level_array, mipmapped_array, level);
    }

    MiniCudaRt::Error_t MiniCudaRt::FreeMipmappedArray(MipmappedArray_t mipmapped_array) const noexcept
    {
        assert(*this && cuda_free_mipmapped_array_);
        return cuda_free_mipmapped_array_(mipmapped_array);
    }

    MiniCudaRt::Error_t MiniCudaRt::Free(void* dev_ptr) const noexcept
    {
